...
with cliente_agentes:
    ...
    with trazador.start_as_current_span("sesion_agente_conversacional") as span_sesion:
        # establece variables de metadata del usuario
        # Esto puede ser útil para personalizar la experiencia del agente
        # o para enviar información adicional a las herramientas del agente.
        # Por ejemplo, puedes incluir un customer_id o cualquier otro dato relevante.
        metadata_usuario = {
            "customer_id": "88129215"
        }
        # PROCESO: Registrar token de usuario en redis con el id customer_id como llave
        ...
        try:
            ...
            agente = cliente_agentes.get_agent('asst_ZCx0jqqxytAJ1t4lpps0GhQ0')
            # Agente Function agente = cliente_agentes.get_agent('asst_dtEhySqpr4w2Mw0DxOqbfhZv')
        ...
        while True:
            with trazador.start_as_current_span("intercambio_conversacional") as span_intercambio:
                ...
                with trazador.start_as_current_span("crear_mensaje_usuario"):
                    # Incluir el customer_id en el contenido del mensaje para que el agente pueda procesarlo                    
                ...
                with trazador.start_as_current_span("ejecutar_agente") as span_ejecucion:              
                    ejecucion = cliente_agentes.runs.create(
                        thread_id=hilo.id, 
                        agent_id=agente.id                    
                    )
                    print(f"🛠️ Ejecución del agente iniciada, ID de ejecución: {ejecucion.id}")
